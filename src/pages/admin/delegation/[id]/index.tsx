...
import { adminApi, DelegationItem, DelegationDetailsSummary, EmployeeResponse, EmployeeDelegation } from "src/lib/api";
...
export default function AdminDelegationDetails() {
  ...
  const fetchDelegationData = async () => {
    ...
    try {
      setLoading(true);
      const data = await adminApi.getDelegationDetails(delegationId);
  ...
  const handleBack = () => {
    // Navigate back to employee view if employee_id is available
    if (delegation?.employee_id) {
      navigate(routes.adminEmployee.replace(':id', delegation.employee_id.toString()));
    } else {
      navigate(-1);
    }
  };
        ...
        {delegation && (
          <S.Subheading>
            ...
            {(delegation.city || delegation.country) && (
              <> • Location: {[delegation.city, delegation.country].filter(Boolean).join(', ')}</>
            )}
          </S.Subheading>
        )}
      </S.Header>
